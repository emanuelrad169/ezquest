# Support Education Prompt

Asset type:
support or education storefront image

Category:
Support and Service

Purpose:
Homepage confidence grid featured support visual: replace placeholder/product technical collage with a clean support-context image.

Canvas:
1200 x 800 px final Shopify-ready image (3:2). Generate at 1536x1024, then crop and resize to the final target.

Composition:
Compact card-friendly scene with one hero USB-C hub and one small support document/device context, no readable text and no clutter.

Environment:
White or very light gray background with minimal surface.

Style:
Premium modern consumer-tech ecommerce image. Apple Store + Xiaomi + Anker-inspired minimal realism. Soft natural lighting, subtle shadows, premium realistic materials. White or very light gray wall background or minimal light wood desk context when useful. No text in image. No clutter. No unnecessary props. Not a product packshot.

Product hierarchy:
Hero object: Slim aluminum USB-C hub as the main object.
Supporting objects:
- One blank folded quick-start card shape or blank tablet edge used only as support context.
...
No diagrams, no readable text, no labels, no fake UI. Keep it calm, bright, and suitable inside a white support card.
