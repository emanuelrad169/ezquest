...
Category:
Desk and Travel Coverage

Purpose:
Homepage feature banner background: communicate ports, power, and display coverage with a premium visual on the left and text-safe space on the right.
...
Product hierarchy:
Hero object: Slim aluminum USB-C dock or hub angled on a clean desk.
Supporting objects:
- One compact matte charger placed smaller and farther back.
- One neatly curved USB-C cable connector.
...
The theme applies a dark overlay, so keep the source image bright, calm, and high contrast enough to survive a dark gradient. No text, no clutter, no plant, no keyboard.
