# Campaign Seasonal Promo Prompt

Asset type:
campaign or seasonal promo asset

Category:
Brand Statement

Purpose:
Homepage cinematic reveal background: premium support-backed product confidence without leaving the section empty.

Canvas:
2400 x 1000 px final Shopify-ready image (12:5). Generate at 1536x1024, then crop and resize to the final target.

Composition:
Simple premium campaign composition with one clear hero object and restrained secondary objects. Preserve large whitespace for promotional copy added in Shopify. Seasonal cues may be present only if requested and must remain subtle, minimal, and non-cluttered.

Environment:
Deep charcoal studio environment with a subtle very dark gray surface, only for this explicitly dark cinematic section.
...
Product hierarchy:
Hero object: Premium satin aluminum USB-C hub placed on the right side.
Supporting objects:
- One subtle cable connector near the hero object.
...
Dark but not dramatic or neon. Soft controlled light, subtle reflections, realistic material, plenty of black negative space on the left. No text in the image.
