...
Purpose:
Support education image: multi-device charging setup with calm, clear product understanding.

Canvas:
1800 x 1200 px final Shopify-ready image (3:2). Generate at 1536x1024, then crop and resize to the final target.

Composition:
One hero multi-port charger with 1-2 connected devices, clean cable routing.
...
Product hierarchy:
Hero object: Multi-port USB-C charger as the dominant object.
Supporting objects:
- One phone with blank screen.
- One tablet or laptop edge with no logo.
...
Keep cables neat and minimal. No screen UI, no labels, no hands.
