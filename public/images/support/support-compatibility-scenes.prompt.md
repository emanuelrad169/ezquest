...
Purpose:
Support education image: compatibility with Mac, iPad, and laptop style devices without showing logos or text.

Canvas:
1800 x 1200 px final Shopify-ready image (3:2). Generate at 1536x1024, then crop and resize to the final target.

Composition:
One hero hub or adapter centered with 1-2 supporting device silhouettes: a laptop and a tablet. No brand logos.
...
Product hierarchy:
Hero object: Compact USB-C adapter or hub as the dominant object.
Supporting objects:
- One silver laptop with no visible logo.
- One tablet with blank screen and no visible logo.
...
No Apple logos, no UI, no screen text, no keyboard focus.
