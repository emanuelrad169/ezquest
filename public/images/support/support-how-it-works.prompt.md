...
Purpose:
Support education image: communicate plug to monitor to power without text, labels, or arrows.

Canvas:
1800 x 1200 px final Shopify-ready image (3:2). Generate at 1536x1024, then crop and resize to the final target.

Composition:
A clear real-world connection scene, not a diagram: one hero USB-C hub connected to a laptop, one monitor, and one power cable.

Environment:
Very light gray wall with clean light wood desk surface.
...
Product hierarchy:
Hero object: USB-C hub with connected cable as the focal point.
Supporting objects:
- One laptop used for context.
- One slim monitor used only to communicate display output.
...
Monitor is allowed for this support concept. Do not add arrows, labels, diagrams, or rendered text.
