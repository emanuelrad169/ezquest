...
Purpose:
Lifestyle context image: charging scene with phone, cable, and charger.

Canvas:
1800 x 1200 px final Shopify-ready image (3:2). Generate at 1536x1024, then crop and resize to the final target.

Composition:
Minimal scene with charger as hero, phone and cable as supporting objects.
...
Product hierarchy:
Hero object: Compact USB-C wall charger as the dominant object.
Supporting objects:
- One modern phone face down or blank screen.
- One neat USB-C cable with no messy loops.
...
Phone screen must be blank or turned away. No UI, no text, no hands.
