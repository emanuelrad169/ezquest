...
Purpose:
Promote docking stations inside the hubs and adapters collection grid.

Canvas:
1200 x 628 px final Shopify-ready image (300:157). Generate at 1536x1024, then crop and resize to the final target.

Composition:
Wide promotional tile. One hero docking station scene centered slightly right, with quiet edge space for dark overlay copy. Keep the subject clear, premium, and not crowded.

Environment:
Clean minimal desk setup with white or very light gray wall background and a light wood or white desk surface.

Primary request:
Premium USB-C docking station on a clean minimal desk setup. MacBook Pro connected to dual monitors, Ethernet cable, and peripherals through one sleek hub. Clean white desk surface, warm ambient light, professional workspace aesthetic. No text. No people. Premium product photography style.
...
Product hierarchy:
Hero object: One sleek USB-C docking station connecting a laptop to a clean desk display setup.
Supporting objects:
- A laptop partly visible as context
- One or two tidy cables only where needed
...
- No visible brand text on screens.
- No messy workstation.
- No keyboard close-up.
- No equal-weight product catalog layout.
