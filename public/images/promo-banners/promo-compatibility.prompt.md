...
Purpose:
Reserve promo image for compatibility messaging across collection grids.

Canvas:
1200 x 628 px final Shopify-ready image (300:157). Generate at 1536x1024, then crop and resize to the final target.

Composition:
Wide promotional tile. One hub is the hero object, devices are supporting context only. Keep the scene minimal and overlay-safe.

Environment:
Clean minimal desk with white or light gray background.

Primary request:
MacBook Pro, Windows laptop, iPad Pro, and iPhone connected through modern USB-C accessories on a clean minimal desk. Multiple devices, one hub, clean workspace. Premium lifestyle photography, light background. No text.
...
Product hierarchy:
Hero object: One USB-C hub as central connector.
Supporting objects:
- Laptop
- Tablet or phone as quiet compatibility context
...
- No visible operating system logos.
- No crowded device pile.
- No people.
