...
Purpose:
Support center hero with approachable technical clarity.

Canvas:
1440 x 480 px final Shopify-ready image (3:1). Generate at 1536x1024, then crop and resize to the final target.

Composition:
Wide light hero. Text-safe left side. Technical object composition on the right.

Environment:
Clean white background with soft blue-gray tones.

Primary request:
Friendly minimal illustration style: a USB-C hub connected to a laptop with subtle glowing port indicators. Clean white background, soft blue-grey tones, technical but approachable aesthetic. No text.
...
Product hierarchy:
Hero object: USB-C hub connected to a laptop.
Supporting objects:
- Subtle glowing port indicators only.
