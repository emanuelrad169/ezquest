...
Asset type:
Homepage feature - Compatibility

Purpose:
Feature section image for multi-platform compatibility.

Canvas:
1200 x 800 px final Shopify-ready image (3:2). Generate at 1536x1024, then crop and resize to the final target.

Composition:
3:2 lifestyle product scene. One USB-C hub as hero connecting three devices in a calm layout.

Environment:
Clean light desk with white or light gray background.

Primary request:
MacBook Pro, iPad Pro, and Windows laptop all connected to one USB-C hub on a clean desk. Lifestyle product photography, light background, multiple devices in harmony. No text.
...
Product hierarchy:
Hero object: USB-C hub.
Supporting objects:
- MacBook Pro.
- iPad Pro and Windows laptop as context.
