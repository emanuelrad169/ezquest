...
Asset type:
Collection hero - Accessories

Purpose:
Collection hero for adapters, card readers, and finishing accessories with text-safe top-left space.

Canvas:
1920 x 600 px final Shopify-ready image (16:5). Generate at 1536x1024, then crop and resize to the final target.

Composition:
Wide 3.2:1 banner. Text-safe top-left area. Flat-lay accessories centered-right with one hero adapter and restrained support objects.

Environment:
White marble or very light stone surface, clean overhead studio lighting.

Primary request:
USB-C adapters, card readers, and small accessories laid flat on a white marble surface. Clean overhead shot, product photography style, light background. No text, no people.
...
Product hierarchy:
Hero object: Premium USB-C adapter or compact card reader.
Supporting objects:
- One smaller adapter.
- One cable connector or card-reader detail.
