...
Purpose:
Resources blog hero for setup guides and product advice.

Canvas:
1440 x 600 px final Shopify-ready image (12:5). Generate at 1536x1024, then crop and resize to the final target.

Composition:
Wide image with text-safe left third and product workspace objects on the right.

Environment:
Light background, clean minimal workspace flat lay with warm tones.

Primary request:
Modern tech workspace flat lay from above: MacBook, USB-C hub, notebook, coffee. Clean minimal lifestyle photography, warm tones, light background. Text-safe left third.
...
Product hierarchy:
Hero object: USB-C hub beside a MacBook.
Supporting objects:
- One notebook and one coffee cup as restrained editorial context.
