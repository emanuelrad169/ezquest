...
Asset type:
Collection hero - Chargers & Power

Purpose:
Collection hero for chargers and power accessories with text-safe top-left space.

Canvas:
1920 x 600 px final Shopify-ready image (16:5). Generate at 1536x1024, then crop and resize to the final target.

Composition:
Wide 3.2:1 banner. Text-safe top-left area. Charger composition centered-right with one hero charger and one supporting cable.

Environment:
Clean white surface with a warm white background and soft studio-natural light.

Primary request:
GaN wall chargers and USB-C cables arranged on a clean white surface. Studio product photography style, warm white background, premium minimalist aesthetic. No text, no people.
...
Product hierarchy:
Hero object: Compact GaN wall charger in matte white or soft gray.
Supporting objects:
- One smaller charger.
- One neatly placed USB-C cable.
