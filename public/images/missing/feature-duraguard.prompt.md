...
Asset type:
Homepage feature - DuraGuard

Purpose:
Feature section image for braided cable durability.

Canvas:
1200 x 800 px final Shopify-ready image (3:2). Generate at 1536x1024, then crop and resize to the final target.

Composition:
3:2 macro product detail. Connector and braid texture are dominant.

Environment:
Dark controlled studio background with dramatic but premium detail light.

Primary request:
Extreme close-up of DuraGuard braided USB-C cable connector. Macro photography, kevlar texture visible, aluminum connector housing, dark background with dramatic studio lighting. Premium product detail.
...
Product hierarchy:
Hero object: DuraGuard-style braided USB-C cable connector.
Supporting objects:
- Visible braided texture only.
