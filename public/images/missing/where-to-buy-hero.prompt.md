...
Purpose:
Where to Buy page hero showing ecommerce availability without in-image text.

Canvas:
1440 x 500 px final Shopify-ready image (72:25). Generate at 1536x1024, then crop and resize to the final target.

Composition:
Wide hero with text-safe left side. Laptop and USB-C products on the right; screen content abstract and text-free.

Environment:
Clean minimal desk setup with light background.

Primary request:
Online shopping concept: a laptop showing an e-commerce interface with USB-C products visible. Clean minimal setup, light background, lifestyle photography. No text on screen.
...
Product hierarchy:
Hero object: Laptop with abstract ecommerce layout on screen.
Supporting objects:
- USB-C products visible beside the laptop.
