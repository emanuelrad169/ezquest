...
Asset type:
Collection index card - Chargers

Purpose:
Full-bleed collection index card visual for chargers and power.

Canvas:
800 x 600 px final Shopify-ready image (4:3). Generate at 1024x1024, then crop and resize to the final target.

Composition:
4:3 full-bleed card. Text-safe bottom third. One hero GaN charger with one supporting cable or charger.

Environment:
Dark surface with restrained moody studio lighting.

Primary request:
GaN USB-C wall chargers plugged into a power strip on a dark surface. Moody studio lighting, premium tech aesthetic. Text-safe bottom third.
...
Product hierarchy:
Hero object: Compact GaN USB-C wall charger.
Supporting objects:
- One power strip detail or USB-C cable.
