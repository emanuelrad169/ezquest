...
Asset type:
Collection hero - Cables

Purpose:
Collection hero for cable quality and durability with text-safe top-left space.

Canvas:
1920 x 600 px final Shopify-ready image (16:5). Generate at 1536x1024, then crop and resize to the final target.

Composition:
Wide 3.2:1 banner. Text-safe top-left area. Cable connectors and coil arranged centered-right with one dominant braided cable.

Environment:
Light gray surface with clean macro studio lighting.

Primary request:
DuraGuard braided USB-C cables coiled and arranged artfully on light grey surface. Macro-style product photography, space gray and black cables, clean minimal background. No text.
...
Product hierarchy:
Hero object: DuraGuard-style braided USB-C cable with aluminum connector housing.
Supporting objects:
- One secondary cable segment or connector placed lower and smaller.
